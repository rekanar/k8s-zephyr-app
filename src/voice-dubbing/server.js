...
// ── API: Save a recorded voice phrase ─────────────────────────────────────────
...
app.delete("/api/phrases/:id", (req, res) => {
  ...
  const filePath = path.join(__dirname, entry.audioPath.replace("/recordings/", "recordings/"));
...
// ── API: AI-powered phrase matching ───────────────────────────────────────────
// Given speech-recognized text, find the best matching recorded phrase
app.post("/api/match", async (req, res) => {
  ...
  const phrases = Array.from(voiceLibrary.values());
  if (phrases.length === 0) {
    return res.json({ match: null, suggestion: spokenText });
  }

  try {
    const phraseList = phrases.map((p) => `ID: ${p.id} | Label: "${p.label}" | Category: ${p.category}`).join("\n");
...
{"matchId": "<id or null>", "confidence": <0-100>, "suggestion": "<cleaned text>"}`,
    ...
    // Extract JSON from response (handle thinking blocks preceding it)
    ...
    res.json({
      match: matchedPhrase ?? null,
      confidence: result.confidence ?? 0,
      suggestion: result.suggestion ?? spokenText,
    });
  } catch (err) {
...
// ── API: AI smart reply suggestion ────────────────────────────────────────────
// Given context, Claude suggests what to say next
...
app.listen(PORT, () => {
  console.log(`Voice Dubbing App running at http://localhost:${PORT}`);
